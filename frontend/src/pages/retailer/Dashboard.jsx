...
const RetailerDashboard = () => {
  ...
  const fetchInventory = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5000/retailers/inventory', {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5000/retailers/notifications', {
        headers: getAuthHeaders(),
      });
  ...
  const handleAddItem = async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/retailers/add_item', {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          name: newItem.name,
  ...
  const handleSellItem = async () => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5000/retailers/inventory/${selectedItem.id}/sell`, {
        method: 'POST',
        headers: getAuthHeaders(),
        body: JSON.stringify({
  ...
  const handleListItem = async (item) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/retailers/inventory/${item.id}/list`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
  ...
  const handleRemoveItem = async (itemId) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:5000/retailers/item/remove/${itemId}`, {
        method: 'DELETE',
        headers: getAuthHeaders(),
      });
  ...
  const handleIgnoreNotification = async (itemId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/retailers/food/${itemId}/ignore`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
