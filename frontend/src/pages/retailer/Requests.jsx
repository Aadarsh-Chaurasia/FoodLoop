...
const RetailerRequests = () => {
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch('http://127.0.0.1:5000/retailers/requested_food', {
  ...
  const handleApproveRequest = async (requestId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/retailers/requests/${requestId}/approve`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
  ...
  const handleIgnoreRequest = async (requestId) => {
    try {
      const response = await fetch(`http://127.0.0.1:5000/retailers/requests/${requestId}/ignore`, {
        method: 'POST',
        headers: getAuthHeaders(),
      });
